...
const TableList: React.FC = () => {
  ...
  let tableColumn = [
    ...
    {
      hidden: !permission_url?.includes('/department/update_update'),
      title: 'Action',
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [
        <Button
          ...
          type="primary"
          onClick={() => {
            history.push('/category/department/update/' + record.id);
          }}
        >
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        headerTitle="Danh sách bộ phận"
        actionRef={actionRef}
        rowKey="id"
        toolBarRender={() => [
          <div key={1}>
            {permission_url?.includes('/department/add_add') && (
              <Button
                type="primary"
                menu={menuProps}
                onClick={() => {
          ...
          <div key={2}>
            {permission_url?.includes('/department/delete_delete') && (
              <Popconfirm
                title="Delete"
