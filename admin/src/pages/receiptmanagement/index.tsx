...
const TableList: React.FC = () => {
  ...
  let tableColumn = [
    ...
    {
      hidden: !permission_url?.includes('/receipt/update_update'),
      title: 'Action',
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [
        <Button
          ...
          type="primary"
          onClick={() => {
            history.push('/storage/receipt/update/' + record.id);
          }}
        >
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        headerTitle="Danh sách hoá đơn nhập"
        actionRef={actionRef}
        rowKey="id"
        toolBarRender={() => [
          <div key={1}>
            {permission_url?.includes('/receipt/add_add') && (
              <Button
                type="primary"
                menu={menuProps}
                onClick={() => {
                  // handleCheckDupModalVisible(true)
                  history.push('/storage/receipt/add');
                }}
              >
          ...
          <div key={2}>
            {permission_url?.includes('/receipt/delete_delete') && (
              <Popconfirm
                title="Delete"
