...
const TableList: React.FC = () => {
  ...
  let tableColumn = [
    ...
    {
      hidden: !permission_url?.includes('/propertyimport/update_update'),
      title: 'Action',
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [
        <Button
          ...
          type="primary"
          onClick={() => {
            history.push('/storage/propertyimport/update/' + record.id);
          }}
        >
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        headerTitle="Danh sách tài sản nhập chi tiết"
        actionRef={actionRef}
        rowKey="id"
        toolBarRender={() => [
          <div key={1}>
            {permission_url?.includes('/propertyimport/add_add') && (
              <Button
                type="primary"
                menu={menuProps}
                onClick={() => {
                  // handleCheckDupModalVisible(true)
                  history.push('/storage/propertyimport/add');
                }}
              >
          ...
          <div key={2}>
            {permission_url?.includes('/propertyimport/delete_delete') && (
              <Popconfirm
                key={2}
                title="Delete"
