...
const TableList: React.FC = () => {
  ...
  let tableColumn = [
    ...
    {
      hidden: !permission_url?.includes('/return/update_update'),
      title: 'Action',
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [
        <Button
          ...
          type="primary"
          onClick={() => {
            history.push('/device/return/update/' + record.id);
          }}
        >
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        headerTitle="Danh sách phiếu trả thiết bị"
        actionRef={actionRef}
        rowKey="id"
        toolBarRender={() => [
          <div key={1}>
            {permission_url?.includes('/return/add_add') && (
              <Button
                type="primary"
                menu={menuProps}
                onClick={() => {
                  // handleCheckDupModalVisible(true)
                  history.push('/device/return/add');
                }}
              >
          ...
          <div key={2}>
            {permission_url?.includes('/return/delete_delete') && (
              <Popconfirm
                title="Delete"
