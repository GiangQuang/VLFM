...
const TableList: React.FC = () => {
  ...
  let tableColumn = [
    ...
    {
      hidden: !permission_url?.includes('/provider/update_update'),
      title: 'Action',
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [
        <Button
          ...
          type="primary"
          onClick={() => {
            history.push('/storage/provider/update/' + record.id);
          }}
        >
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        ...
        toolBarRender={() => [
          <div key={1}>
            {permission_url?.includes('/provider/add_add') && (
              <Button
                type="primary"
                menu={menuProps}
                onClick={() => {
                  // handleCheckDupModalVisible(true)
                  history.push('/storage/provider/add');
                }}
              >
          ...
          <div key={2}>
            {permission_url?.includes('/provider/delete_delete') && (
              <Popconfirm
                key={2}
                title="Delete"
