...
const TableList: React.FC = () => {
  ...
  let tableColumn = [
    ...
    {
      hidden: !permission_url?.includes('/user/update_update'),
      title: 'Action',
      dataIndex: 'option',
      valueType: 'option',
      render: (_, record) => [
        <Button
          ...
          type="primary"
          onClick={() => {
            history.push('/category/user/update/' + record.id);
          }}
        >
  ...
  return (
    <PageContainer>
      <ProTable<TableListItem, TableListPagination>
        ...
        toolBarRender={() => [
          <div key={1}>
            {permission_url?.includes('/user/add_add') && (
              <Button
                type="primary"
                menu={menuProps}
                onClick={() => {
                  // handleCheckDupModalVisible(true)
                  history.push('/category/user/add');
                }}
              >
          ...
          <div key={2}>
            {permission_url?.includes('/user/delete_delete') && (
              <Popconfirm
                title="Delete"
